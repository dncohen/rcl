...
secret keys. While not strictly speaking a requirement when using the
testnet, RCL tools expects this will all keys. So let's go through the
motions.

    rcl-key backup *.rcl-key

The `rcl-key backup` operation walks you through the secrets, giving you a
chance to copy each one to paper. Don't bother for test accounts, just press
[return] a few times and you'll see how `rcl-key backup` works.
